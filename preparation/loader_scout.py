...
def _machine_name_to_power_characteristics(machine_type: str) -> Tuple[float, float]:
    """Returns the static and maximum power usage of an EC2 instance type."""
    try:
        type_, size = machine_type.split("4")
    except ValueError:
    ...
    return p_static * factor, p_max * factor


def _resolve_energy(machine_name: str, node_metrics: Dict[str, NodeMetricsModel]) -> float:
    """Returns the workloads energy usage in Wh."""
    p_static, p_max = _machine_name_to_power_characteristics(machine_name)
    cpu_utilization_values: List[float] = []  # between 0 and 1
    for node_metric in node_metrics.values():
        for record in node_metric.cpu_busy:
            cpu_utilization_values.append(record.value / 100)
    power_usage_values = p_static + np.array(cpu_utilization_values) * (p_max - p_static)  # linear interpolation
    energy_usage_values = power_usage_values * 5 / 3600  # each record covers 5 seconds
    return float(np.sum(energy_usage_values))
...
def _load_processed_workload(tuple_list: List[Tuple[ScoutName, str]]) -> Optional[ProcessedWorkloadModel]:
    ...
    if re_workload is not None:
        ...
        re_workload.energy = _resolve_energy(re_workload.machine_name, re_workload.node_metrics)
