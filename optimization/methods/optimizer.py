...
class OptimizerBO:
    ...
    def __init__(self, task: WorkloadTask, runtime_target: float, profile_num: int, **kwargs):
        ...
        self.selection_strategy: Optional[AbstractSelectionStrategy] = None

    def init(self):
        # for some randomly chosen initial workloads, train the model initially
        while len([w for w in self.profiled_workloads if not w.abandon]) < self.num_init:
            workload: Optional[ProcessedWorkloadModel] = handle_sobol_suggestion(self.ax_client, self.task.workloads,
    ...
    def profile(self, workload: ProcessedWorkloadModel, optional_properties: Optional[dict] = None):
        ...
        self.dict_list.append({
            **self.base_entry,
            ...
            "selected_tasks": [t.json(exclude={"workloads"}) for t in self.selection_strategy.selected_tasks]
            if self.selection_strategy is not None else [],
            **optional_properties
        })
    ...
    def _run(self):
        ...
        observation_features = [ObservationFeatures(self.embedder.construct(self.embedder.vectorize(w)))
        ...
        acqf_values = [
            ...
            for observation_feature in observation_features
        ]
