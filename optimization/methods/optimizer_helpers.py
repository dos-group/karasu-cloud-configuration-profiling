...
def get_moo_objectives(evaluator: EvaluatorBO, task: WorkloadTask):
    ...
    return {
        "objective_cost": ObjectiveProperties(
            minimize=True,
            # bound: max. theoretical scale-out x max. theoretical unit cost x max runtime
            threshold=threshold_adapter(evaluator.cost_transform(task.lower_cost_bound(evaluator.runtime_target)))),
        "objective_energy": ObjectiveProperties(
            minimize=True,
            # worst case, all metrics show the lowest utilization
            threshold=threshold_adapter(evaluator.energy_transform(task.lower_energy_bound(evaluator.runtime_target))))
    }
...
def setup_moo_experiment(name: str, gs: GenerationStrategy, embedder, evaluator, task):
    ...
    ax_client.create_experiment(
        ...
        objectives=get_moo_objectives(evaluator, task),
